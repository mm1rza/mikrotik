...
set [ find default-name=ether2 ] name=ether2-matrik
set [ find default-name=ether4 ] name=ether4-mikrotik
...
add bridge=bridge-lan interface=ether4-mikrotik
...
add default-route-distance=2 disabled=no interface=ether2-matrik
...
add action=mark-routing chain=prerouting comment=isp2 dst-address-list=\
    z-list-raw-wa new-routing-mark=isp2 passthrough=no src-address=\
    192.168.9.0/24
add action=mark-routing chain=prerouting comment=isp2 dst-port=\
    !21-23,53,80-81,443,853,5353,8000,8008,8080,8081,8090,8443,8888 \
    new-routing-mark=isp2 passthrough=no protocol=tcp src-address=\
    192.168.9.0/24
add action=mark-routing chain=prerouting comment=isp2 dst-port=\
    !21-23,53,80-81,443,853,5353,8000,8008,8080,8081,8090,8443,8888 \
    new-routing-mark=isp2 passthrough=no protocol=udp src-address=\
    192.168.9.0/24
...
add action=masquerade chain=srcnat out-interface=ether2-matrik
/ip firewall raw
add action=add-dst-to-address-list address-list=z-list-raw-wa \
    address-list-timeout=none-dynamic chain=prerouting content=.whatsapp.com \
...
add action=add-dst-to-address-list address-list=z-list-raw-wa \
    ...
    dst-address-list=!ip-local-private src-address=192.168.9.0/24
/ip route
add comment="isp2 manual" distance=1 gateway=192.168.1.1 routing-mark=isp2
add comment=isp1 distance=1 gateway=192.168.100.1 routing-mark=isp
add comment=isp2 distance=2 gateway=192.168.1.1 routing-mark=isp
...
add comment="  isp2 cek" down-script="/ip route disable [find comment=\"isp2\"\
    ...
    \n/ip firewall mangle disable [find comment=\"isp2\"]" host=\
    149.112.112.11 interval=10s up-script="/ip route enable [find comment=\"is\
    p2\"]\r\
