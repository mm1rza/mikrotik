# jun/22/2023 12:00:00 by RouterOS 6.48.4
...
/ip dhcp-server
add address-pool=dhcp_pool0 disabled=no interface=bridge2 lease-script=\
    "/queue simple remove [ find name ~\"hs-<\" ]" name=dhcp1
add address-pool=dhcp_pool1 disabled=no interface=ether10 lease-script=\
    "/queue simple remove [ find name ~\"hs-<\" ]" name=dhcp2
...
add address-pool=dhcp_pool4 disabled=no interface=bridge-13-langsungan \
    lease-script=":local queueName \"<free_\$leaseActMAC>\";\
    \n:if (\$leaseBound = \"1\") do={\
    \n/queue simple add name=\$queueName target=(\$leaseActIP . \"/32\") max-l\
    imit=5000k/5000k parent=\"~~~GLOBAL TRAFIK\" place-before=\"~~~GLOBAL TRAF\
    IK\";\
    \n} else={\
...
add address=192.168.20.1/22 interface=bridge1 network=192.168.20.0
